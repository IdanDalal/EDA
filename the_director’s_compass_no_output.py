...
<div style="text-align: center;">
    <img src="https://tovtech.org/wp-content/uploads/2024/02/%D7%A2%D7%95%D7%AA%D7%A7-%D7%A9%D7%9C-%D7%9C%D7%95%D7%92%D7%95-%D7%98%D7%95%D7%91%D7%98%D7%A7-%D7%9C%D7%9C%D7%90-%D7%98%D7%A7%D7%A1%D7%98.png" width="300" height="500"/>
</div>
...
imdb_raw = pd.read_csv('/kaggle/input/top-500-600-movies-of-each-year-from-1960-to-2024/final_dataset.csv')
...
pv(f"Loaded dataset with <span style='color:{LOGO};font-weight:bold'>{len(imdb_raw):,}</span> rows and "
   ...
   f" to <span style='color:{LOGO};font-weight:bold'>{int(imdb_raw['year'].max()) if imdb_raw['year'].notna().any() else 'n/a'}</span>.")
...
**<u>Claim:</u>** The distribution of audience ratings is not uniform. It is left-skewed, with most films clustering in the 6.0 to 7.5 range.
...
if "rating" in imdb_raw.columns:
    ...
    fig = dark(fig, title="Ratings skew positively with 63.5% (37,567) of films rated between 5.5 and 7.25", height=HEIGHT_MED, width=WIDTH)
...
if show_cols:
    display(sty(
        imdb.head(8)[show_cols].rename(columns={"duration_minutes": "minutes"}),
...
To ensure commercial relevancy, we will filter the dataset to include only "visible films", those that have achieved a minimum threshold of audience engagement.

We define this as any film with at least 1,000 votes.

This 1,000-vote visibility filter removes approximately **45%** of the raw dataset but retains films that represent over 95% of total audience engagement.
...
directors_visible  = directors[directors["votes_n"] >= 1_000].copy()

# Verification message
pv(f"Created final analysis set with <span style='color:{LOGO};font-weight:bold'>{len(directors_visible ):,}</span>"
   f" film-director entries (from films with >= 1,000 votes).")
...
if "duration_minutes" in imdb.columns:
    ...
    fig = dark(fig, title="60% of all films and 71.9% of films with >10k votes are within the commercial norm", height=HEIGHT_MED, width=WIDTH)
...
**<u>Evidence:</u>** The charts show that **Drama** and **Comedy** are the most produced genres.
...
fig = make_subplots(
    ...
    subplot_titles=("Output - top 10 genres by film count: Drama & Comedy dominate with >40%",
                    "Median rating - audience quality benchmark: Drama, Romance, & Crime rate highly",
                    "Median votes - audience reach benchmark: Action & Adventure drive engagement"))
...
fig = dark(fig, title="These top 10 genres represent 94% of all films, excluded ones would add <1% each", height=HEIGHT_TALL, width=WIDTH)
fig.show(renderer='iframe')
...
fig = dark(fig, title="<span style='font-weight:bold; font-style:italic;'>Specialists</span> can match <span style='font-weight:bold; font-style:italic;'>Masters</span> in Rating & Success despite gaps in Votes & Diversity", height=HEIGHT_MED, width=WIDTH)
fig.show(renderer='iframe')
...
**<u>Evidence:</u>** The shortlist above identifies the top 10 directors from ***The Masters*** quadrant, a group that consistently demonstrates elite performance across quality, reach, output, and versatility.
...
1. **Immediate** (Q1): Greenlight projects with top 5 ***Masters***.
2. **Near-term** (Q2-Q3): Develop ***Specialist***-led genre films.
3. **Strategic** (Q4+): Pilot innovative projects with high-potential ***Explorers***.
4. **Ongoing**: Track ***Craftsmen*** for breakout signals.
"""
...
pv(f"Shortlist size: <span style='color:{LOGO};font-weight:bold'>{len(shortlist):,}</span>.<br>"
   f"Success score range: <span style='color:{LOGO};font-weight:bold'>{shortlist['success_score'].max():.1f}</span>"
   f" to <span style='color:{LOGO};font-weight:bold'>{shortlist['success_score'].min():.1f}</span>.")
